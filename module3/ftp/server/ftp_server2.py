    ...
    def _put(self):
        ...
        while filecount < filesize:
            ...
            if put_file_md5.decode('utf-8') != local_file_md5:
                ...
                self.logger.info('[%s] File md5 value is correct. [%s]' % \
                                    (str(self.client_address), filename))

    def _get(self):
        expect_file = replace_relative_path(self.basedir, self.current_dir, self.recv)
        if not os.path.exists(expect_file):
            self.request.sendall(bytes('File not found', encoding='utf-8'))  # File Not Found
            return False
        ...
        try:
            with open(expect_file, 'rb') as f:
                ...
                for line in f:
                    ...
                    md5_value = file_md5.hexdigest()
                    self.request.sendall(bytes(md5_value, encoding='utf-8'))
                    self.logger.info('[%s] Transafer "%s" complete.' % \
